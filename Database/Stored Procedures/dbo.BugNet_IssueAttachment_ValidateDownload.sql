...
AS
	...
	SET @AttachmentExists =
	  (
		  SELECT COUNT(*)FROM [BNIssueAttachments] WHERE IssueAttachmentId = @IssueAttachmentId
	  );
	...
	SET @AnonymousAccess =
	  (
		  ...
		  FROM
			  [BNHostSettings]
		  WHERE
			  LOWER(SettingName) = 'anonymousaccess'
	  );
...
*/
	...
	FROM
		[dbo].[BugNet_IssuesView] i
		INNER JOIN [BNIssueAttachments] ia ON [i].[IssueId] = ia.IssueId
		INNER JOIN [dbo].[BNProjects] p ON [i].[ProjectId] = [p].[ProjectId]
	WHERE
		ia.IssueAttachmentId = @IssueAttachmentId
		AND (
				[i].[Disabled] = 0
				AND [p].[ProjectDisabled] = 0
			);
...
*/
	...
	SET @HasPermission =
	  (
		  ...
		  FROM
			  [dbo].[BNUserRoles] ur
			  INNER JOIN [dbo].[BNRoles] r ON [ur].[RoleId] = [r].[RoleId]
		  WHERE
			  (
				  [r].[ProjectId] = @ProjectId
				  OR [r].[ProjectId] IS NULL
			  )
			  AND (
					  LOWER([r].[RoleName]) = @ProjectAdminString
					  OR [r].[RoleId] = 1
				  )
	...
	SET @HasProjectAccess =
	  (
		  ...
		  FROM
			  [dbo].[BNUserProjects]
		  WHERE
			  [BNUserProjects].[UserId] = @RequestingUserID
...
*/
	...
	SELECT
		@ReturnValue = ia.IssueAttachmentId
	FROM
		[dbo].[BugNet_IssuesView] i
		INNER JOIN [BNIssueAttachments] ia ON [i].[IssueId] = ia.IssueId
		INNER JOIN [dbo].[BNProjects] p ON [i].[ProjectId] = [p].[ProjectId]
	WHERE
		...
		AND (
				( @HasPermission = 1 )
				OR -- requesting user has elevated permissions
				(
					@IssueVisibility = 0
					AND @AnonymousAccess = 1
				)
				OR -- issue is visible and anon access is on
				(
					( @IssueVisibility = 1 )
					AND -- issue is private
					(
						( UPPER([i].[IssueCreatorUserId]) = UPPER(@RequestingUserID))
						OR -- requesting user is issue creator
						(
							UPPER([i].[IssueAssignedUserId]) = UPPER(@RequestingUserID)
							AND @AnonymousAccess = 0
						) -- requesting user is assigned to issue and anon access is off 
					)
					OR ( @IssueVisibility = 0 ) -- issue is visible show it
				)
			)
		AND (
				[i].[Disabled] = 0
